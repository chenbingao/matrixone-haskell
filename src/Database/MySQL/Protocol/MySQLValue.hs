...
import Blaze.Text qualified as Textual
...
import Data.ByteString.Lex.Fractional qualified as LexFrac
import Data.ByteString.Lex.Integral qualified as LexInt
...
data MySQLValue
  ...
  | -- | DATE
    MySQLDate !Day
  | -- | sign(0 = non-negative, 1 = negative) hh mm ss microsecond
...
getTextField f
  ...
  | t == mySQLTypeDecimal
      || t == mySQLTypeNewDecimal =
      feedLenEncBytes t MySQLDecimal fracLexer
  | t == mySQLTypeTiny =
      if isUnsigned
        then feedLenEncBytes t MySQLInt8U intLexer
        else feedLenEncBytes t MySQLInt8 intLexer
  | t == mySQLTypeShort =
      if isUnsigned
        then feedLenEncBytes t MySQLInt16U intLexer
        else feedLenEncBytes t MySQLInt16 intLexer
  | t == mySQLTypeLong
      || t == mySQLTypeInt24 =
      if isUnsigned
        then feedLenEncBytes t MySQLInt32U intLexer
        else feedLenEncBytes t MySQLInt32 intLexer
  | t == mySQLTypeLongLong =
      if isUnsigned
        then feedLenEncBytes t MySQLInt64U intLexer
  ...
  | t == mySQLTypeDate
      || t == mySQLTypeNewDate =
      feedLenEncBytes t MySQLDate dateParser
  | t == mySQLTypeTime
  ...
  | t == mySQLTypeVarChar
      ...
      || t == mySQLTypeUUID
      || t == mySQLTypeString =
      (if isText then MySQLText . T.decodeUtf8 else MySQLBytes) <$> getLenEncBytes
...
feedLenEncBytes typ con parser = do
  ...
  case parser bs of
    ...
    Nothing ->
      fail $
        "Database.MySQL.Protocol.MySQLValue: parsing "
          ++ show typ
          ++ " failed, \
             \input: "
          ++ BC.unpack bs
...
putTextField (MySQLDateTime dt) =
  putInQuotes $
    putByteString (BC.pack (formatTime defaultTimeLocale "%F %T%Q" dt))
putTextField (MySQLTimeStamp dt) =
  putInQuotes $
    putByteString (BC.pack (formatTime defaultTimeLocale "%F %T%Q" dt))
putTextField (MySQLDate d) =
  putInQuotes $
...
putTextField (MySQLText t) =
  putInQuotes $
    putByteString . T.encodeUtf8 . escapeText $
      t
...
getBinaryField f
  ...
  | t == mySQLTypeVarChar
      ...
      || t == mySQLTypeString =
      if isText
        then MySQLText . T.decodeUtf8 <$> getLenEncBytes
        else MySQLBytes <$> getLenEncBytes
  | t == mySQLTypeBit = MySQLBit <$> (getBits =<< getLenEncInt)
  | otherwise =
      fail $
        "Database.MySQL.Protocol.MySQLValue:\
        \ missing binary decoder for "
          ++ show t
  where
    t = columnType f
    isUnsigned = flagUnsigned (columnFlags f)
...
getBits bytes =
  if
    | bytes == 0 || bytes == 1 -> fromIntegral <$> getWord8
...
putBinaryField (MySQLDecimal n) =
  putLenEncBytes . L.toStrict . BB.toLazyByteString $
...
putBinaryField (MySQLYear n) =
  putLenEncBytes . L.toStrict . BB.toLazyByteString $
    Textual.integral n -- this's really weird, it's not documented anywhere
    -- we must encode year into string in binary mode!
putBinaryField (MySQLTimeStamp (LocalTime date time)) = do
  putWord8 11 -- always put full
  putBinaryDay date
  putBinaryTime' time
putBinaryField (MySQLDateTime (LocalTime date time)) = do
  putWord8 11 -- always put full
  putBinaryDay date
  putBinaryTime' time
putBinaryField (MySQLDate d) = do
  putWord8 4
  putBinaryDay d
putBinaryField (MySQLTime sign t) = do
  putWord8 12 -- always put full
  putWord8 sign
  putBinaryTime t
putBinaryField (MySQLGeometry bs) = putLenEncBytes bs
putBinaryField (MySQLBytes bs) = putLenEncBytes bs
putBinaryField (MySQLBit word) = do
  putWord8 8 -- always put full
  putWord64be word
...
putBinaryTime' (TimeOfDay hh mm ss) = do
  let s = floor ss
