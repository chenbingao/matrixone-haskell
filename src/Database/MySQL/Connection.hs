...
import Data.Connection qualified as TCP
import Data.IORef
  ( IORef,
    newIORef,
    readIORef,
...
import System.IO.Streams.TCP qualified as TCP
...
decodeInputStream is = Stream.makeInputStream $ do
  ...
  let len =
        fromIntegral (bs `B.unsafeIndex` 0)
...
writeCommand a writePacket =
  let bs = Binary.runPut (putCommand a)
   in go (L.length bs) 0 bs writePacket
  where
...
data NetworkException = NetworkException deriving (Show)
